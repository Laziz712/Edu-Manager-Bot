...
bot.command('myid', async (ctx) => {
  await ctx.reply(
    `🆔 Sizning Telegram ID'ingiz: \`${ctx.from.id}\`\n\nAdmin bo'lish uchun shu ID'ni \`.env\` faylidagi \`ADMIN_IDS\` ga qo'shing va botni qayta ishga tushiring.`,
    { parse_mode: 'Markdown' }
  );
});
...
bot.command('users', async (ctx) => {
  if (!isAdmin(ctx)) {
    return ctx.reply(
      `❌ Sizda admin huquqlari yo'q.\n\n🆔 Sizning ID'ingiz: \`${ctx.from.id}\`\nShu ID'ni .env faylidagi ADMIN_IDS ga qo'shing va botni qayta ishga tushiring.`,
      { parse_mode: 'Markdown' }
    );
  }
